  ...
  const CampaignDetail: React.FC = () => {
    ...
    useEffect(() => {
      if (id) {
        ...
        // Intentar conectar WebSocket (si está disponible en el backend)
        ...
        if (WS_URL) {
          try {
            ...
            // Suscribirse a esta campaña
            wsService.subscribeToCampaign(id);

            // Escuchar eventos de nuevos votos
            const handleVoteCast = (data: any) => {
              if (data.campaignId === id) {
                console.log('Nuevo voto recibido en tiempo real');
                loadCampaignData();
              }
            };

            // Escuchar cambios en la campaña
            const handleCampaignUpdated = (data: any) => {
              if (data.campaignId === id) {
                console.log('Campaña actualizada en tiempo real');
                loadCampaignData();
              }
            };
          ...
          // Fallback: Si no hay WebSocket, usar setInterval
              ...
              {campaign.candidatos.map((candidate) => (
                <div
                  ...
                  className={`candidate-card ${selectedCandidate === candidate._id ? 'selected' : ''} ${
                    hasVoted || campaign.estado !== 'habilitada' ? 'disabled' : ''
                  }`}
                  onClick={() => {
